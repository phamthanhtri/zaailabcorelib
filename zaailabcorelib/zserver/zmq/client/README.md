...
```python
from bert_serving.client import WKRClient
bc = WKRClient()
...
> - [Using `WKRClient` with `tf.data` API](#using-bertclient-with-tfdata-api)
...
> - [Starting `WKRServer` from Python](#starting-bertserver-from-python)
...
[Please always refer to the latest client-side API documented here.](https://bert-as-service.readthedocs.io/en/latest/source/client.html#module-client) Client-side provides a Python class called `WKRClient`, which accepts arguments as follows:
...
A `WKRClient` implements the following methods and properties:
...
The full list of examples can be found in [`example/`](example). You can run each via `python example/example-k.py`. Most of examples require you to start a WKRServer first, please follow [the instruction here](#2-start-the-bert-service). Note that although `WKRClient` works universally on both Python 2.x and 3.x, examples are only tested on Python 3.6.
...
This gives `33 questions loaded, avg. len of 9`. So looks like we have enough questions. Now start a WKRServer with `uncased_L-12_H-768_A-12` pretrained BERT model:
```bash
...
### Using `WKRClient` with `tf.data` API
...
The [`tf.data`](https://www.tensorflow.org/guide/datasets) API enables you to build complex input pipelines from simple, reusable pieces. One can also use `WKRClient` to encode sentences on-the-fly and use the vectors in a downstream model. Here is an example:
...
bc_clients = [WKRClient(show_server_config=False) for _ in range(num_clients)]
...
The trick here is to start a pool of `WKRClient` and reuse them one by one. In this way, we can fully harness the power of `num_parallel_calls` of `Dataset.map()` API.  
...
`WKRClient.encode()` offers a nice synchronous way to get sentence encodes. However,   sometimes we want to do it in an asynchronous manner by feeding all textual data to the server first, fetching the encoded results later. This can be easily done by:
```python
...
```
, where `id` is a unique identifier helping you to synchronize the results; `is_tokenized` follows the meaning in [`WKRClient` API](https://bert-as-service.readthedocs.io/en/latest/source/client.html#client.WKRClient.encode_async) and `false` by default.
...
### Starting `WKRServer` from Python

Besides shell, one can also start a `WKRServer` from python. Simply do
```python
from bert_serving.server.helper import get_args_parser
from bert_serving.server import WKRServer
...
To shutdown the server, you may call the static method in `WKRServer` class via:
```python
WKRServer.shutdown(port=5555)
```
...
**A:** Server side no, client side yes. This is based on the consideration that python 2.x might still be a major piece in some tech stack. Migrating the whole downstream stack to python 3 for supporting `bert-as-service` can take quite some effort. On the other hand, setting up `WKRServer` is just a one-time thing, which can be even [run in a docker container](#run-bert-service-on-nvidia-docker). To ease the integration, we support python 2 on the client side so that you can directly use `WKRClient` as a part of your python 2 project, whereas the server side should always be hosted with python 3.
...
##### **Q:** I encounter `zmq.error.ZMQError: Operation cannot be accomplished in current state` when using `WKRClient`, what should I do?

**A:** This is often due to the misuse of `WKRClient` in multi-thread/process environment. Note that you can’t reuse one `WKRClient` among multiple threads/processes, you have to make a separate instance for each thread/process. For example, the following won't work at all:
...
It's even worse if you put `WKRClient()` inside the loop. Don't do that.
